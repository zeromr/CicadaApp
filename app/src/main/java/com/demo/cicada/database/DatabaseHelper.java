package com.demo.cicada.database;
...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    //数据库名
    private static final String DATABASE_NAME = "MusicDatabase.weather";
    ...
    //音乐表建表语句
    private String createMusicTable = "create table if not exists " + MUSIC_TABLE + "(" + ID_COLUMN + " integer " +
            "PRIMARY KEY ," + NAME_COLUMN + " text," + SINGER_COLUMN + " text," + ALBUM_COLUMN + " text," +
            DURATION_COLUMN + " long," + PATH_COLUMN + " text," + PARENT_PATH_COLUMN + " text," + LOVE_COLUMN + " " +
            "integer," + FIRST_LETTER_COLUMN + " text );";

    //创建播放历史表
    private String createLastPlayTable = "create table if not exists " + LAST_PLAY_TABLE + " (" + ID_COLUMN + " " +
            "integer," + "FOREIGN KEY(id) REFERENCES " + MUSIC_TABLE + " (id) ON DELETE CASCADE);";

    //创建歌单表
    private String createPlaylistTable = "create table if not exists " + PLAY_LIST_TABLE + " (" + ID_COLUMN + " " +
            "integer PRIMARY KEY autoincrement," + NAME_COLUMN + " text);";

    //创建歌单歌曲表
    private String createListinfoTable = "create table if not exists " + PLAY_LISY_MUSIC_TABLE + " (" + ID_COLUMN + "" +
            " integer," + MUSIC_ID_COLUMN + " integer," + "FOREIGN KEY(id) REFERENCES " + PLAY_LIST_TABLE + "(id) ON " +
            "DELETE CASCADE," + "FOREIGN KEY(music_id) REFERENCES " + MUSIC_TABLE + " (id) ON DELETE CASCADE) ;";
